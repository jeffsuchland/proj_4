...
var pizzaElementGenerator = function(i) {
  ...
  pizzaContainer.classList.add("small");
...
var resizePizzas = function(size) { 
  ...
  // New vesrion reduces number of DOM searches and instead adds or removes classes based on what size of pizza.
  function changePizzaSizes(size) {
	  var windowwidth = document.getElementById("randomPizzas").offsetWidth;
	  var randPizList = document.getElementsByClassName("randomPizzaContainer");
    for (var i = 0; i < randPizList.length; i++) {
	var newwidth = size;

      if (newwidth === "1") {
      	      randPizList[i].classList.remove("med");
	      randPizList[i].classList.remove("large");
	      randPizList[i].classList.add("small");
      }
      if (newwidth === "2") {
	      randPizList[i].classList.remove("small");
	      randPizList[i].classList.remove("large");
	      randPizList[i].classList.add("med");
      }
      if (newwidth === "3") {
      	      randPizList[i].classList.remove("small");
	      randPizList[i].classList.remove("med");
	      randPizList[i].classList.add("large");
      }
    }
  }
...
// My new version moves repeated calculations out of for loops to improve speed.
function updatePositions() {
  ...
  for (var i =0; i < 5; i++) {
	  phases[i] = Math.sin(scrolled + (i % 5));
  }
